...
const Pill = ({ children, c='var(--ink-3)', bg='transparent', bold=false, small=false }) => (
  <span style={{
...
const HealthOrb = ({ health='on-track', progress=0, size=14 }) => {
...
const ProgressBar = ({ v=0, c='var(--accent)', h=4 }) => (
...
const Btn = ({ children, onClick, variant='ghost', size='md', icon, style, title }) => {
  const base = {
...
const Card = ({ children, style, pad=16, title, right, onClick }) => (
...
const Sparkbars = ({ values=[], h=22, c='var(--accent)' }) => (
...
const DepThread = ({ x1,y1,x2,y2, kind='blocks' }) => {
  const colors = { blocks:'var(--risk)', feeds:'var(--accent)', enables:'#7a52c0' };
...
Object.assign(window, { Avatar, AvatarStack, Dot, Pill, StatusPill, HealthOrb, ProgressBar, Kbd, Btn, Card, Sparkbars, DepThread });
