...
const ProjectView = ({ id, setView }) => {
  ...
  return (
    <div style={{padding:'20px 28px', maxWidth:1280, margin:'0 auto'}}>
      ...
      <div style={{
        ...
        <div style={{display:'flex', alignItems:'flex-start', gap:16}}>
          ...
          <div style={{flex:1, minWidth:0}}>
            ...
            <div style={{fontSize:14, color:'var(--ink-2)', marginTop:6, maxWidth:720, textWrap:'pretty'}}>{p.summary}</div>
          </div>
      ...
      {tab==='docs' && <ProjectDocs p={p}/>}
      {tab==='activity' && <ProjectActivity/>}
    </div>
  );
};

const ProjectOverview = ({ p, tasks, deps }) => (
  <div style={{display:'grid', gridTemplateColumns:'1.5fr 1fr', gap:18}}>
    <div style={{display:'flex',flexDirection:'column',gap:18}}>
      ...
      {p.blockers && (
        <Card style={{background:'color-mix(in oklab, var(--risk) 6%, white)', borderColor:'color-mix(in oklab, var(--risk) 30%, var(--line))'}} title="Blockers">
          {p.blockers.map((b,i)=>(
...
const TaskList = ({ tasks }) => (
  <Card pad={0}>
    ...
    {['in-progress','review','todo','blocked','done'].map(st => {
