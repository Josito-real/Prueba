...
const Roadmap = ({ openProject }) => {
  ...
  const headerH = 48;
  const width = 1060;
  const pctX = (iso) => ((new Date(iso).getTime()-startDate)/span)*width;
  ...
  // lookup for deps
  const boxOf = id => boxes.find(b=>b.p.id===id);
...
Object.assign(window, { Roadmap, CapacityHeatBand });
