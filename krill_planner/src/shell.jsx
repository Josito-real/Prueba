...
const CommandRail = ({ view, setView, openProject, activeProject, openPalette, onLogoClick }) => {
...
const RailBtn = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} title={label} style={{
    ...
    {React.createElement(Icons[icon]||Icons.dot,{size:16})}
  </button>
);

const TopBar = ({ view, activeProject, crumb, onCmd, extra }) => {
...
const CommandPalette = ({ open, onClose, onAction }) => {
  ...
  React.useEffect(()=>{ if(open) setTimeout(()=>ref.current?.focus(), 30); setQ(''); }, [open]);
  if (!open) return null;

  const items = [
    ...NAV_SECTIONS.map(s => ({ kind:'view', id:s.id, label:`Go to ${s.label}`, icon:s.icon, hint:'view' })),
    ...PROJECTS.map(p => ({ kind:'project', id:p.id, label:p.name, sub:p.code, icon:'folder', hint:'project' })),
    ...TASKS.slice(0,8).map(t => ({ kind:'task', id:t.id, label:t.title, sub:t.id, icon:'check', hint:'task' })),
        ...
        <div style={{overflowY:'auto'}}>
          {filtered.slice(0,14).map((i,idx)=>(
            <button key={idx} onClick={()=>{ onAction(i); onClose(); }} style={{
              ...
              <span style={{color:'var(--ink-3)'}}>{React.createElement(Icons[i.icon]||Icons.dot,{size:15})}</span>
              <span style={{flex:1}}>{i.label}{i.sub && <span style={{color:'var(--ink-4)', marginLeft:8, fontFamily:'var(--font-mono)', fontSize:11}}>{i.sub}</span>}</span>
...
Object.assign(window, { CommandRail, TopBar, CommandPalette, NAV_SECTIONS });
