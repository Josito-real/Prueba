...
const PulledTasks = () => {
  const tasks = TODAY.pull.map(id => TASKS.find(t=>t.id===id)).filter(Boolean);
  return (
  ...
  <Card title="Needs attention" right={<span style={{fontSize:11,color:'var(--ink-4)'}}>auto-curated</span>}>
    {[
      ...
      <div key={i} style={{display:'flex',alignItems:'flex-start', gap:12, padding:'10px 0', borderTop:i?'1px solid var(--line-2)':'0'}}>
        <span style={{color:r.c, marginTop:2}}>{React.createElement(Icons[r.icon],{size:16})}</span>
...
const TodayClassic = ({ openProject }) => (
...
const ActiveProjectsRow = ({ openProject }) => (
  <div>
    ...
    <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:14}}>
      {PROJECTS.slice(0,6).map(p=>(
        <Card key={p.id} pad={14} onClick={()=>openProject(p.id)} style={{transition:'border-color .12s'}}
          ...
          <div style={{fontSize:11.5, color:'var(--ink-3)', marginBottom:12, lineHeight:1.5, minHeight:32, textWrap:'pretty'}}>{p.summary}</div>
...
Object.assign(window, { TodayStrip, TodayClassic, TodayFocus, fmtTime });
